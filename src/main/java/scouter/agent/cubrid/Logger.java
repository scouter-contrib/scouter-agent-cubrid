...
public class Logger {
    ...
    public static void systemDebugPrintln(Object message) {
        if (AlertConfigure.getInstance().agent_debuging) {
            System.out.println(DateUtil.datetime(System.currentTimeMillis()) + " " + message);
        }
    }

    public static void systemPrintln(Object message) {
        System.out.println(DateUtil.datetime(System.currentTimeMillis()) + " " + message);
    }
    ...
    private static boolean checkOk(String id, int sec) {
        if (Configure.getInstance().mgr_log_ignore_ids.hasKey(id)) return false;
        if (sec > 0) {
    ...
    private static void println(String msg) {
        try {
            ...
            Logger.systemPrintln(msg);
        }
    }

    private static Runnable proc =
            new Runnable() {
    ...
    private static synchronized void openFile() throws IOException {
        if (pw == null) {
            ...
            if (conf.log_rotation_enalbed) {
                FileWriter fw =
                        new FileWriter(
                                new File(conf.log_dir, "agent-" + DateUtil.yyyymmdd() + ".log"),
                                true);
    ...
    protected static void clearOldLog() {
        if (conf.log_rotation_enalbed == false) return;
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (name.startsWith("agent-") == false) continue;
    ...
    public static void main(String[] args) {
        ...
        int x = name.lastIndexOf('.');
        String date = name.substring("agent-".length(), x);
        Logger.systemDebugPrintln(date);
    }
}
